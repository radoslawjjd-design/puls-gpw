...
- [ ] 1.1 E2E login tests pass: `uv run pytest tests/e2e/test_login_ux.py`
- [ ] 1.2 No regression in adjacent E2E suites (`test_idle_timeout.py`, `test_url_routing.py`, `test_x_post_history.py`, `test_profile_menu.py`)
- [ ] 1.3 Full E2E suite green: `uv run pytest tests/e2e/`

#### Manual

- [ ] 1.4 Banner renders correctly at desktop width
- [ ] 1.5 Banner scales correctly at narrow/mobile width without overflow or overlap
- [ ] 1.6 Box corners remain rounded (banner doesn't bleed past border-radius)
- [ ] 1.7 Login flow (valid key + invalid key error) still works end-to-end
- [ ] 1.8 No console errors related to the new image
