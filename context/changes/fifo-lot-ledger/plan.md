...
the instrument name resolved as **first non-`None`**, and the acquisition instant of
the oldest still-open lot.
...
- The module is pure — `uv run python -c "import ast,sys;src=open('src/portfolio_lots.py').read();assert 'bigquery' not in src and 'fastapi' not in src"`
...
- Linting passes: `uv run ruff check .`
- No lot consumption left in this module: `git grep -n "lots\[ticker\]" src/portfolio_realized.py` returns nothing

#### Manual Verification:

- `GET /api/portfolio/realized` returns the same figures as before the change, with the two new fields present and plausible against a known two-tranche ticker
...
- Full suite passes: `uv run pytest --tb=short`
- Linting passes: `uv run ruff check .`
- No lot machinery left outside the ledger: `git grep -n "_consume_oldest\|_Lot"` matches only `src/portfolio_lots.py` and its tests
...
- An XTB export parsed through the rebuilt path yields the same positions and closed tickers as before
...
- [ ] 1.1 New ledger tests pass
- [ ] 1.2 Nothing else regressed (full suite)
- [ ] 1.3 Linting passes
- [ ] 1.4 The module is pure (no bigquery/fastapi import)

#### Manual

- [ ] 1.5 Public shape can carry part 2's as-of-date query without restructuring
...
- [ ] 2.6 Realized endpoint returns unchanged figures plus plausible days-held
...
- [ ] 3.6 A parsed XTB export yields the same positions and closed tickers
