...
**File**: `context/changes/fifo-lot-ledger/baseline/` (new, git-tracked)
...
**Contract**: Two JSON artefacts committed in this phase — the output of
`compute_realized_pnl` over the production trade history (unfiltered, plus one
year-filtered run), and the `(positions, closed_tickers)` produced by
`reconstruct_positions` over the XTB fixture used by `tests/test_brokers_xtb.py`. Both
are generated by a short throwaway script run against the current code; only the
artefacts are committed, not the script. They are what criteria 2.6 and 3.6 diff
against, and they are deleted in the archive step.
...
- Baseline artefacts exist and are non-empty: `test -s context/changes/fifo-lot-ledger/baseline/realized.json && test -s context/changes/fifo-lot-ledger/baseline/positions.json`
...
- `reconstruct_positions` over the XTB fixture diffs clean against `baseline/positions.json`
...
- [ ] 1.3 Linting passes
- [ ] 1.4 The module imports nothing outside the standard library
- [ ] 1.5 Baseline artefacts exist and are non-empty
...
- [ ] 3.6 reconstruct_positions diffs clean against baseline/positions.json
