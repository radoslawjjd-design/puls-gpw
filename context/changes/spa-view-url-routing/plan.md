...
- [ ] 1.1 Full test suite passes: `uv run pytest --tb=short`
- [ ] 1.2 E2E suite passes: `uv run pytest tests/e2e/ -q`

#### Manual

- [ ] 1.3 Clicking "Treemapa portfela" sets the URL to `...?view=treemap`
- [ ] 1.4 Clicking "Historia postów X" sets the URL to `...?view=x-history`
- [ ] 1.5 Clicking the "puls-gpw" topbar heading returns the URL to `/` (no `view=`)
- [ ] 1.6 Refreshing while on `?view=treemap` lands back on the treemap view, not announcements
- [ ] 1.7 Browser back after Treemapa → Historia postów X returns to the treemap view (URL shows `?view=treemap` again)
...
- [ ] 2.2 E2E suite passes: `uv run pytest tests/e2e/ -q`
- [ ] 2.3 `tests/e2e/test_pagination.py` passes unmodified
- [ ] 2.4 `tests/e2e/test_refresh.py` passes (with the Phase 4 assertion additions)

#### Manual

- [ ] 2.5 Setting a ticker filter and paging to page 2 shows both `ticker=` and `page=2` in the URL
- [ ] 2.6 Refreshing on that URL restores the ticker filter field's value, the event-type dropdown's label, and page 2's data
- [ ] 2.7 Setting a date filter, refreshing, re-opening the date field shows the same date/time (not shifted by timezone)
- [ ] 2.8 An old-style bookmark (`?page=2&page_size=20`, no filters, no `view=`) still lands on announcements page 2 with empty filters
...
- [ ] 4.1 New `tests/e2e/test_url_routing.py` passes (view-switch URLs, back-navigation, deep link, refresh, old-format bookmark)
- [ ] 4.2 `tests/e2e/test_portfolio_treemap.py` updated assertions pass (view URL on open, popup close leaves URL unchanged)
- [ ] 4.3 `tests/e2e/test_x_post_history.py` updated assertions pass (view URL on open, page URL on next)
- [ ] 4.4 `tests/e2e/test_refresh.py::test_refresh_preserves_page_and_filters` passes

#### Manual

- [ ] 4.5 Log in, click through all three views via the menu, confirm each shows the expected URL
- [ ] 4.6 Filters + page 2 on announcements reproduce in a fresh private window via copied URL
- [ ] 4.7 Browser back twice on x-history after paging forward twice steps back through x-history pages, not into announcements
- [ ] 4.8 Treemap popup open + Escape leaves the URL unchanged and does not consume an extra back-press
