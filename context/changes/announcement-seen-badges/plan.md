...
- [ ] 1.1 Full test suite passes: `uv run pytest`
- [ ] 1.2 Wiring sanity: `_markViewSeen` hooked in router/url-state/doLogout/pagehide/visibilitychange; `_seenThreshold` read-only (grep)

#### Manual

- [ ] 1.3 Headed browser: badge render → popup-clear (instant + after reload)
- [ ] 1.4 Navigate-away / logout / tab-hide advance thresholds correctly
- [ ] 1.5 Both views, light + dark, no console errors

### Phase 2: E2E coverage (4 core scenarios)

#### Automated

- [ ] 2.1 New spec passes: `uv run pytest tests/e2e/test_new_badges.py`
- [ ] 2.2 Full suite passes with fixture changes: `uv run pytest`
